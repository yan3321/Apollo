...
export default class Bot {
    ...
    private maxRefreshFails = Environment.get<number>(
            ...
            this.server.queryServer().then((query) => {
                if (query) {
                    ...
                    if (this.refreshFails >= this.maxRefreshFails) {
                        ...
                        console.error(
                            "Failed to refresh server info, emitting error."
                        );
    ...
    constructor() {
        ...
        this.server = new Server(
            Environment.get("ip"),
            Environment.get<number>("port", "number")
        );
    ...
    private onMessage(message: Message) {
        ...
        if (
            message.content ===
            Environment.get<string | undefined>(
                "refresh_command",
                "string",
                true
            )
            ...
            message.content ===
            Environment.get("maintenance_toggle_command", "string", true)
        ) {
            if (
                message.member &&
                this.discord.doesUserHaveServerManagerPermissions(
                    message.member
                )
            ) {
    ...
    private async toggleMaintenanceMode(message: Message) {
        ...
        if (this.maintenanceMode) {
            ...
            const embed = await this.discord.createRichEmbed(
                undefined,
    ...
    private refreshBotWithRolePermissions(message: Message) {
        if (
            message.member &&
            this.discord.doesUserHaveServerManagerPermissions(message.member)
        ) {
            this.refresh(true);
        } else {
            this.replyNoPermissions(message);
    ...
    private replyNoPermissions(message: Message) {
        if (Environment.get<boolean>("reply_dm_on_no_perms", "boolean")) {
            message.author.send(
                `${message.member} ${Environment.locale.noPermissions}`
            );
        } else {
    ...
    private async refresh(forceNewMessage = false) {
        ...
        if (messageId && !forceNewMessage) {
            this.discord
                .editMessage(
                    messageId,
                    await this.discord.createRichEmbed(query)
                )
                .then(() => {
                    this.discord.setActivity(
                        query ? "ok" : "serverError",
                        query
                    );
                    this.discord.stopThinking();
                })
                .catch((error: Error) => {
                    ...
                    console.error(
                        `Failed to edit current message, id: ${messageId}.`
                    );
                    console.error(error);
                });
        } else {
            ...
            this.discord
                .postMessage(await this.discord.createRichEmbed(query))
                .then((newId: string) => {
        ...
        if (query) {
            const minPlayers = Environment.get<number>(
            ...
            if (query.players.length >= minPlayers) {
                ...
                if (!Settings.get().pingMessageId) {
                    const pingMessageId = await this.discord.postMessage(
                        this.discord.generatePing(
                            Environment.get("reaction_role_id")
    ...
    private refreshLoop() {
        const timeToWait =
            Environment.get<number>("time_to_check_minutes", "number") *
            1000 *
            60;
